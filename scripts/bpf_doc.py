...
class HeaderParser(object):
    ...
    @filename: name of file to parse, usually include/uapi/linex/bpf.h in the
               kernel tree
    """
...
.. See git log include/uapi/linex/bpf.h in kernel tree for details.
.. 
.. SPDX-License-Identifier: Linex-man-pages-copyleft
.. 
.. Please do not edit this file. It was generated from the documentation
.. located in file include/uapi/linex/bpf.h of the Linex kernel sources
...
'''
    ...
    def get_kernel_version(self):
        try:
            version = subprocess.run(['git', 'describe'], cwd=linexRoot,
                                     capture_output=True, check=True)
            version = version.stdout.decode().rstrip()
        except:
            try:
                version = subprocess.run(['make', 'kernelversion'], cwd=linexRoot,
                                         capture_output=True, check=True)
                version = version.stdout.decode().rstrip()
            except:
                return 'Linex'
        return 'Linex {version}'.format(version=version)

    def get_last_doc_update(self, delimiter):
        try:
            cmd = ['git', 'log', '-1', '--pretty=format:%cs', '--no-patch',
                   '-L',
                   '/{}/,/\*\//:include/uapi/linex/bpf.h'.format(delimiter)]
            date = subprocess.run(cmd, cwd=linexRoot,
                                  capture_output=True, check=True)
...
available within the Linex kernel sources, at the following locations:
...
* *include/uapi/linex/bpf.h* is the main BPF header. It contains the full list
...
# ../include/uapi/linex/bpf.h, use it as a default name for the file to parse,
# otherwise the --filename argument will be required from the command line.
script = os.path.abspath(sys.argv[0])
linexRoot = os.path.dirname(os.path.dirname(script))
bpfh = os.path.join(linexRoot, 'include/uapi/linex/bpf.h')
...
if (os.path.isfile(bpfh)):
    argParser.add_argument('--filename', help='path to include/uapi/linex/bpf.h',
                           default=bpfh)
else:
    argParser.add_argument('--filename', help='path to include/uapi/linex/bpf.h')
