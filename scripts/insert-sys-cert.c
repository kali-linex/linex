 ...
 *
 * Author: Mehmet Kayaalp <mkayaalp@linex.vnet.ibm.com>
 *
 * This software may be used and distributed according to the terms
 * of the GNU General Public License, incorporated herein by reference.
 *
 * Usage: insert-sys-cert [-s <System.map> -b <vmlinex> -c <certfile>
 */
...
{
	printf("Usage %s [-s <System.map>] -b <vmlinex> -c <certfile>\n", e);
}

int main(int argc, char **argv)
{
	char *system_map_file = NULL;
	char *vmlinex_file = NULL;
	char *cert_file = NULL;
	int vmlinex_size;
	int cert_size;
	Elf_Ehdr *hdr;
	...
	hdr = map_file(vmlinex_file, &vmlinex_size);
	if (!hdr)
		exit(EXIT_FAILURE);

	if (vmlinex_size < sizeof(*hdr)) {
