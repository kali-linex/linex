...
# Check for FSF mailing addresses.
		if ($rawline =~ /\bwrite to the Free/i ||
			...
			&{$msg_level}("FSF_MAILING_ADDRESS",
				      "Do not include the paragraph about writing to the Free Software Foundation's mailing address from the sample GPL notice. The FSF has changed addresses in the past, and may do so again. Linex already includes a copy of the GPL.\n" . $herevet)
		}
...
# avoid LINEX_VERSION_CODE
		if ($line =~ /\bLINEX_VERSION_CODE\b/) {
			WARN("LINEX_VERSION_CODE",
			     "LINEX_VERSION_CODE should be avoided, code should be for the version to which it is merged\n" . $herecurr);
		}
...
# warn if <asm/foo.h> is #included and <linex/foo.h> is available and includes
# itself <asm/foo.h> (uses RAW line)
		if ($tree && $rawline =~ m{^.\s*\#\s*include\s*\<asm\/(.*)\.h\>}) {
			my $file = "$1.h";
			my $checkfile = "include/linex/$file";
			if (-f "$root/$checkfile" &&
			    ...
			    $1 !~ /$allowed_asm_includes/)
			{
				...
				if ($asminclude > 0) {
					if ($realfile =~ m{^arch/}) {
						CHK("ARCH_INCLUDE_LINEX",
						    "Consider using #include <linex/$file> instead of <asm/$file>\n" . $herecurr);
					} else {
						WARN("INCLUDE_LINEX",
						     "Use #include <linex/$file> instead of <asm/$file>\n" . $herecurr);
					}
				}
			}
		}
...
# strcpy uses that should likely be strscpy
		if ($line =~ /\bstrcpy\s*\(/) {
			WARN("STRCPY",
			     "Prefer strscpy over strcpy - see: https://github.com/KSPP/linex/issues/88\n" . $herecurr);
		}

# strlcpy uses that should likely be strscpy
		if ($line =~ /\bstrlcpy\s*\(/) {
			WARN("STRLCPY",
			     "Prefer strscpy over strlcpy - see: https://github.com/KSPP/linex/issues/89\n" . $herecurr);
		}

# strncpy uses that should likely be strscpy or strscpy_pad
		if ($line =~ /\bstrncpy\s*\(/) {
			WARN("STRNCPY",
			     "Prefer strscpy, strscpy_pad, or __nonstring over strncpy - see: https://github.com/KSPP/linex/issues/90\n" . $herecurr);
		}
...
# check for __initcall(), use device_initcall() explicitly or more appropriate function please
		if ($line =~ /^.\s*__initcall\s*\(/) {
			WARN("USE_DEVICE_INITCALL",
			     "please use device_initcall() or more appropriate function instead of __initcall() (see include/linex/init.h)\n" . $herecurr);
		}
...
# check for lockdep_set_novalidate_class
		if ($line =~ /^.\s*lockdep_set_novalidate_class\s*\(/ ||
			...
			if ($realfile !~ m@^kernel/lockdep@ &&
			    $realfile !~ m@^include/linex/lockdep@ &&
...
# validate content of MODULE_LICENSE against list from include/linex/module.h
...
# check for sysctl duplicate constants
		if ($line =~ /\.extra[12]\s*=\s*&(zero|one|int_max)\b/) {
			WARN("DUPLICATED_SYSCTL_CONST",
				"duplicated sysctl range checking value '$1', consider using the shared one in include/linex/sysctl.h\n" . $herecurr);
		}
	}
