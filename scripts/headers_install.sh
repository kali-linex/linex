...
# SPDX-License-Identifier with GPL variants must have "WITH Linex-syscall-note"
if [ -n "$(sed -n -e "/SPDX-License-Identifier:.*GPL-/{/WITH Linex-syscall-note/!p}" $INFILE)" ]; then
	echo "error: $INFILE: missing \"WITH Linex-syscall-note\" for SPDX-License-Identifier" >&2
	exit 1
fi
...
sed -E -e '
	...
	s@^#include <linex/compiler(|_types).h>@@
