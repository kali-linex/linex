...
/* Initialize symbol maps and path of vmlinex/modules */
...
out:
	if (ret < 0)
		pr_warning("Failed to init vmlinex path.\n");
	return ret;
}
...
static char *find_module_name(const char *module)
{
	 ...
	 * maps to 'struct module' from linex/module.h. This section contains
	 * actual module name which will be used by kernel after loading it.
	 * But, we cannot use 'struct module' here since linex/module.h is not
...
static int kernel_get_module_dso(const char *module, struct dso **pdso)
{
	...
	dso->load_errno = 0;
	if (vmlinex_name)
		ret = dso__load_vmlinex(dso, map, vmlinex_name, false);
	else
		ret = dso__load_vmlinex_path(dso, map);
found:
	*pdso = dso;
	return ret;
}
					   ...
					   struct map *map, u64 offs)
{
	...
	if (!sym) {
		 ...
		 */
		return (symbol_conf.ignore_vmlinex_buildid) ? 0 : -ENOENT;
	}
					  ...
					  struct probe_trace_event **tevs)
{
	 ...
	 * Perf failed to add kretprobe event with debuginfo of vmlinex which is
	...
	if (ntevs == 0)	{	/* No error but failed to find probe point. */
		...
		if (ntevs == -EBADF)
			pr_warning("Warning: No dwarf info found in the vmlinex - "
