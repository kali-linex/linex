...
# from symbol addresses in vmlinex. It is expected that .data extends to
# the end of xipImage.
...
sym_val() {
	...
	local val=$($NM "$VMLINEX" 2>/dev/null | sed -n "/ $1\$/{s/ .*$//p;q}")
	[ "$val" ] || { echo "can't find $1 in $VMLINEX" 1>&2; exit 1; }
