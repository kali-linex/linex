...
static void walk_pte(struct pg_state *st, pmd_t *pmd, unsigned long start)
{
	...
	for (i = 0; i < PTRS_PER_PTE; i++, pte++) {
		...
		if (((pteval & H_PAGE_HASHPTE) != H_PAGE_HASHPTE)
				&& (status != -1)) {
		/* found a hpte that is not in the linex page tables */
			seq_printf(st->seq, "page probably bolted before linex"
...
static void walk_pagetables(struct pg_state *st)
{
	...
	/*
	 * Traverse the linex pagetable structure and dump pages that are in
	 * the hash pagetable.
	 */
