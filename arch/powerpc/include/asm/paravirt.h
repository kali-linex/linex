...
static inline bool vcpu_is_preempted(int cpu)
{
	...
	if (!is_kvm_guest()) {
		 ...
		 * by events internal and external to Linex. While we can
		 * be called in preemptable context (in the Linex sense),
		 * we're not accessing per-cpu resources in a way that can
		 * race destructively with Linex scheduler preemption and
