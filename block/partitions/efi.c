 ...
 * - Changed gpt structure names and members to be simpler and more Linex-like.
 * 
 ...
 * - Moved crc32() to linex/lib, added efi_crc32().
 *
 ...
 *
 * We do not create a Linex partition for GPT, but
...
int efi_partition(struct parsed_partitions *state)
{
	...
	for (i = 0; i < le32_to_cpu(gpt->num_partition_entries) && i < state->limit-1; i++) {
		...
		if (!efi_guidcmp(ptes[i].partition_type_guid, PARTITION_LINEX_RAID_GUID))
