 ...
 *    we need to do that using the IS_* macros defined in include/linex/fs.h.
...
no_data_attr_special_case:
	 ...
	 * consistent with the Linex Way. Doing it the Linex Way would cause a
	 * significant slowdown as it would involve iterating over all
	 * attributes in the mft record and adding the allocated/compressed
	 * sizes of all non-resident attributes present to give us the Linex
	...
	if (a->flags & (ATTR_COMPRESSION_MASK | ATTR_IS_SPARSE)) {
		...
		if (NInoMstProtected(ni) && ni->type != AT_INDEX_ROOT) {
			ntfs_error(vi->i_sb, "Found mst protected attribute "
					...
					"linex-ntfs-dev@lists.sourceforge.net",
	...
	if (a->flags & ATTR_IS_ENCRYPTED) {
		...
		if (NInoMstProtected(ni) && ni->type != AT_INDEX_ROOT) {
			ntfs_error(vi->i_sb, "Found mst protected attribute "
					...
					"to linex-ntfs-dev@lists.sourceforge."
					"net");
			goto unm_err_out;
		}
		if (ni->type != AT_DATA) {
...
retry_truncate:
	...
	if (new_size < vol->mft_record_size &&
		 ...
		 * Given both POSIX and the Linex mmap(2) man page specify that
