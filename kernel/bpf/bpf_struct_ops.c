 ...
 * stored at the map->btf_vmlinex_value_type_id.
 *
 */
...
#undef BPF_STRUCT_OPS_TYPE
	...
	if (module_id < 0) {
		pr_warn("Cannot find struct module in btf_vmlinex\n");
		return;
	}
	...
	for (i = 0; i < ARRAY_SIZE(bpf_struct_ops); i++) {
		...
		value_id = btf_find_by_name_kind(btf, value_name,
						 BTF_KIND_STRUCT);
		if (value_id < 0) {
			pr_warn("Cannot find struct %s in btf_vmlinex\n",
				value_name);
			continue;
		}
...
static int check_zero_holes(const struct btf_type *t, void *data)
{
	...
	for_each_member(i, t, member) {
		...
		mtype = btf_type_by_id(btf_vmlinex, member->type);
		mtype = btf_resolve_size(btf_vmlinex, mtype, &msize);
		if (IS_ERR(mtype))
					   ...
					   void *value, u64 flags)
{
	...
	for_each_member(i, t, member) {
		...
		moff = __btf_member_bit_offset(t, member) / 8;
		ptype = btf_type_resolve_ptr(btf_vmlinex, member->type, NULL);
					     ...
					     struct seq_file *m)
{
	...
	if (!err) {
		btf_type_seq_show(btf_vmlinex, map->btf_vmlinex_value_type_id,
				  value, m);
		seq_puts(m, "\n");
	}
...
static int bpf_struct_ops_map_alloc_check(union bpf_attr *attr)
{
	if (attr->key_size != sizeof(unsigned int) || attr->max_entries != 1 ||
	    (attr->map_flags & ~BPF_F_LINK) || !attr->btf_vmlinex_value_type_id)
		return -EINVAL;
	return 0;
}
...
static struct bpf_map *bpf_struct_ops_map_alloc(union bpf_attr *attr)
{
	...
	st_ops = bpf_struct_ops_find_value(attr->btf_vmlinex_value_type_id);
	if (!st_ops)
