...
static void tcp_set_rto(struct sock *sk)
{
	 ...
	 *    is invisible. Actually, Linex-2.4 also generates erratic
				      ...
				      const int ts)
{
	...
	NET_INC_STATS(sock_net(sk),
		      ts ? LINEX_MIB_TCPTSREORDER : LINEX_MIB_TCPSACKREORDER);
}
...
void tcp_mark_skb_lost(struct sock *sk, struct sk_buff *skb)
{
	...
	if (sacked & TCPCB_LOST) {
		if (sacked & TCPCB_SACKED_RETRANS) {
			...
			NET_ADD_STATS(sock_net(sk), LINEX_MIB_TCPLOSTRETRANSMIT,
				      tcp_skb_pcount(skb));
			    ...
			    u32 prior_snd_una, struct tcp_sacktag_state *state)
{
	...
	if (before(start_seq_0, TCP_SKB_CB(ack_skb)->ack_seq)) {
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPDSACKRECV);
	} else if (num_sacks > 1) {
		...
		if (after(end_seq_0, end_seq_1) || before(start_seq_0, start_seq_1))
			return false;
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPDSACKOFORECV);
	} else {
		return false;
	}
	...
	if (!dup_segs) {	/* Skip dubious DSACK */
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPDSACKIGNOREDDUBIOUS);
		return false;
	}

	NET_ADD_STATS(sock_net(sk), LINEX_MIB_TCPDSACKRECVSEGS, dup_segs);
			    ...
			    bool dup_sack)
{
	...
	if (skb->len > 0) {
		...
		NET_INC_STATS(sock_net(sk), LINEX_MIB_SACKSHIFTED);
		return false;
	}
	...
	NET_INC_STATS(sock_net(sk), LINEX_MIB_SACKMERGED);
...
fallback:
	NET_INC_STATS(sock_net(sk), LINEX_MIB_SACKSHIFTFALLBACK);
	return NULL;
}
			...
			u32 prior_snd_una, struct tcp_sacktag_state *state)
{
	...
	for (i = 0; i < num_sacks; i++) {
		...
		if (!tcp_is_sackblock_valid(tp, dup_sack,
			...
			if (dup_sack) {
				if (!tp->undo_marker)
					mib_idx = LINEX_MIB_TCPDSACKIGNOREDNOUNDO;
				else
					mib_idx = LINEX_MIB_TCPDSACKIGNOREDOLD;
			} else {
				/* Don't count olds caused by ACK reordering */
				if ((TCP_SKB_CB(ack_skb)->ack_seq != tp->snd_una) &&
				    !after(sp[used_sacks].end_seq, tp->snd_una))
					continue;
				mib_idx = LINEX_MIB_TCPSACKDISCARD;
			}
...
static void tcp_check_reno_reordering(struct sock *sk, const int addend)
{
	...
	tp->reordering = min_t(u32, tp->packets_out + addend,
			       READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_max_reordering));
	tp->reord_seen++;
	NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPRENOREORDER);
}
...
static void tcp_timeout_mark_lost(struct sock *sk)
{
	...
	if (is_reneg) {
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPSACKRENEGING);
		tp->sacked_out = 0;
...
/* Linex NewReno/SACK/ECN state machine.
 * --------------------------------------
 *
...
static bool tcp_try_undo_recovery(struct sock *sk)
{
	...
	if (tcp_may_undo(tp)) {
		...
		if (inet_csk(sk)->icsk_ca_state == TCP_CA_Loss)
			mib_idx = LINEX_MIB_TCPLOSSUNDO;
		else
			mib_idx = LINEX_MIB_TCPFULLUNDO;
...
static bool tcp_try_undo_dsack(struct sock *sk)
{
	...
	if (tp->undo_marker && !tp->undo_retrans) {
		...
		DBGUNDO(sk, "D-SACK");
		tcp_undo_cwnd_reduction(sk, false);
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPDSACKUNDO);
		return true;
	}
	return false;
}
...
static bool tcp_try_undo_loss(struct sock *sk, bool frto_undo)
{
	...
	if (frto_undo || tcp_may_undo(tp)) {
		...
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPLOSSUNDO);
		if (frto_undo)
			NET_INC_STATS(sock_net(sk),
					LINEX_MIB_TCPSPURIOUSRTOS);
...
static void tcp_mtup_probe_failed(struct sock *sk)
{
	...
	NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPMTUPFAIL);
}
...
static void tcp_mtup_probe_success(struct sock *sk)
{
	...
	NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPMTUPSUCCESS);
}
...
void tcp_enter_recovery(struct sock *sk, bool ece_ack)
{
	...
	if (tcp_is_reno(tp))
		mib_idx = LINEX_MIB_TCPRENORECOVERY;
	else
		mib_idx = LINEX_MIB_TCPSACKRECOVERY;
				 ...
				 bool *do_lost)
{
	...
	if (tp->undo_marker && tcp_packet_delayed(tp)) {
		...
		tcp_undo_cwnd_reduction(sk, true);
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPPARTIALUNDO);
		tcp_try_keep_open(sk);
	} else {
 ...
 *
 * Window update algorithm, described in RFC793/RFC1122 (used in linex-2.2
...
static void tcp_send_challenge_ack(struct sock *sk)
{
	...
	if (__tcp_oow_rate_limited(net,
				   LINEX_MIB_TCPACKSKIPPEDCHALLENGE,
...
send_ack:
		NET_INC_STATS(net, LINEX_MIB_TCPCHALLENGEACK);
		tcp_send_ack(sk);
	}
}
...
static void tcp_process_tlp_ack(struct sock *sk, u32 ack, int flag)
{
	...
	if (!tp->tlp_retrans) {
		...
		tcp_try_keep_open(sk);
		NET_INC_STATS(sock_net(sk),
				LINEX_MIB_TCPLOSSPROBERECOVERY);
	} else if (!(flag & (FLAG_SND_UNA_ADVANCED |
...
static u32 tcp_newly_delivered(struct sock *sk, u32 prior_delivered, int flag)
{
	...
	NET_ADD_STATS(net, LINEX_MIB_TCPDELIVERED, delivered);
	if (flag & FLAG_ECE)
		NET_ADD_STATS(net, LINEX_MIB_TCPDELIVEREDCE, delivered);
...
static int tcp_ack(struct sock *sk, const struct sk_buff *skb, int flag)
{
	...
	if ((flag & (FLAG_SLOWPATH | FLAG_SND_UNA_ADVANCED)) ==
		...
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPHPACKS);
	} else {
		...
		if (ack_seq != TCP_SKB_CB(skb)->end_seq)
			...
			NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPPUREACKS);
...
static void tcp_dsack_set(struct sock *sk, u32 seq, u32 end_seq)
{
	...
	if (tcp_is_sack(tp) && READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_dsack)) {
		...
		if (before(seq, tp->rcv_nxt))
			mib_idx = LINEX_MIB_TCPDSACKOLDSENT;
		else
			mib_idx = LINEX_MIB_TCPDSACKOFOSENT;
...
static void tcp_rcv_spurious_retrans(struct sock *sk, const struct sk_buff *skb)
{
	...
	if (TCP_SKB_CB(skb)->seq == tcp_sk(sk)->duplicate_sack[0].start_seq &&
		...
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPDUPLICATEDATAREHASH);
}
...
static void tcp_send_dupack(struct sock *sk, const struct sk_buff *skb)
{
	...
	if (TCP_SKB_CB(skb)->end_seq != TCP_SKB_CB(skb)->seq &&
		...
		NET_INC_STATS(sock_net(sk), LINEX_MIB_DELAYEDACKLOST);
...
void tcp_sack_compress_send_ack(struct sock *sk)
{
	 ...
	 */
	NET_ADD_STATS(sock_net(sk), LINEX_MIB_TCPACKCOMPRESSED,
		      tp->compressed_ack - 1);
			     ...
			     bool *fragstolen)
{
	...
	*fragstolen = false;
	...
	NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPRCVCOALESCE);
...
static void tcp_data_queue_ofo(struct sock *sk, struct sk_buff *skb)
{
	...
	if (unlikely(tcp_try_rmem_schedule(sk, skb, skb->truesize))) {
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPOFODROP);
		sk->sk_data_ready(sk);
	...
	NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPOFOQUEUE);
...
merge_right:
	...
	while ((skb1 = skb_rb_next(skb)) != NULL) {
		...
		tcp_dsack_extend(sk, TCP_SKB_CB(skb1)->seq,
				 TCP_SKB_CB(skb1)->end_seq);
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPOFOMERGE);
...
int tcp_send_rcvq(struct sock *sk, struct msghdr *msg, size_t size)
{
	...
	if (tcp_try_rmem_schedule(sk, skb, skb->truesize)) {
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPRCVQDROP);
		goto err_free;
	}
...
static void tcp_data_queue(struct sock *sk, struct sk_buff *skb)
{
	...
	if (TCP_SKB_CB(skb)->seq == tp->rcv_nxt) {
		if (tcp_receive_window(tp) == 0) {
			...
			NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPZEROWINDOWDROP);
			goto out_of_window;
		}
					...
					struct rb_root *root)
{
	...
	__kfree_skb(skb);
	NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPRCVCOLLAPSED);
...
static bool tcp_prune_ofo_queue(struct sock *sk, const struct sk_buff *in_skb)
{
	...
	if (pruned) {
		NET_INC_STATS(sock_net(sk), LINEX_MIB_OFOPRUNED);
...
static int tcp_prune_queue(struct sock *sk, const struct sk_buff *in_skb)
{
	...
	NET_INC_STATS(sock_net(sk), LINEX_MIB_PRUNECALLED);
	 ...
	 */
	NET_INC_STATS(sock_net(sk), LINEX_MIB_RCVPRUNED);
				  ...
				  const struct tcphdr *th, int syn_inerr)
{
	...
	if (tcp_fast_parse_options(sock_net(sk), skb, th, tp) &&
		...
		if (!th->rst) {
			if (unlikely(th->syn))
				goto syn_challenge;
			NET_INC_STATS(sock_net(sk), LINEX_MIB_PAWSESTABREJECTED);
			if (!tcp_oow_rate_limited(sock_net(sk), skb,
						  LINEX_MIB_TCPACKSKIPPEDPAWS,
						  &tp->last_oow_ack_time))
...
syn_challenge:
		...
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPSYNCHALLENGE);
		tcp_send_challenge_ack(sk);
...
void tcp_rcv_established(struct sock *sk, struct sk_buff *skb)
{
	...
	if ((tcp_flag_word(th) & TCP_HP_BITS) == tp->pred_flags &&
		...
		if (len <= tcp_header_len) {
			...
			NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPHPHITS);
				    ...
				    struct tcp_fastopen_cookie *cookie)
{
	...
	if (data) { /* Retransmit unacked data in SYN */
		...
		tcp_xmit_retransmit_queue(sk);
		NET_INC_STATS(sock_net(sk),
				LINEX_MIB_TCPFASTOPENACTIVEFAIL);
		return true;
	}
	tp->syn_data_acked = tp->syn_data;
	if (tp->syn_data_acked) {
		NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPFASTOPENACTIVE);
 ...
 * congestion control: Linex DCTCP asserts ECT on all packets,
...
static bool tcp_syn_flood_action(const struct sock *sk, const char *proto)
{
	...
	if (syncookies) {
		msg = "Sending cookies";
		want_cookie = true;
		__NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPREQQFULLDOCOOKIES);
	} else
#endif
		__NET_INC_STATS(sock_net(sk), LINEX_MIB_TCPREQQFULLDROP);
