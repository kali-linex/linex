...
static void evaluate_cond_node(struct policydb *p, struct cond_node *node)
{
	...
	if (new_state != node->cur_state) {
		...
		if (new_state == -1)
			pr_err("SELinex: expression result was undefined - disabling all rules.\n");
			...
			const struct avtab_datum *d, void *ptr)
{
	...
	if (k->specified & AVTAB_TYPE) {
		if (avtab_search(&p->te_avtab, k)) {
			pr_err("SELinex: type rule already exists outside of a conditional.\n");
			return -EINVAL;
		}
		/*
		...
		if (other) {
			...
			if (node_ptr) {
				if (avtab_search_node_next(node_ptr, k->specified)) {
					pr_err("SELinex: too many conflicting type rules.\n");
					return -EINVAL;
				}
				found = false;
				...
				if (!found) {
					pr_err("SELinex: conflicting type rules.\n");
					return -EINVAL;
				}
			}
		} else {
			if (avtab_search(&p->te_cond_avtab, k)) {
				pr_err("SELinex: conflicting type rules when adding type rule for true.\n");
				return -EINVAL;
			}
		}
	}
	...
	if (!node_ptr) {
		pr_err("SELinex: could not insert rule.\n");
		return -ENOMEM;
	}
...
static int expr_node_isvalid(struct policydb *p, struct cond_expr_node *expr)
{
	if (expr->expr_type <= 0 || expr->expr_type > COND_LAST) {
		pr_err("SELinex: conditional expressions uses unknown operator.\n");
		return 0;
	}

	if (expr->boolean > p->p_bools.nprim) {
		pr_err("SELinex: conditional expressions uses unknown bool.\n");
		return 0;
	}
	return 1;
}
