...
#if CONFIG_SECURITY_SELINEX_SID2STR_CACHE_SIZE > 0
	s->cache_free_slots = CONFIG_SECURITY_SELINEX_SID2STR_CACHE_SIZE;
			  ...
			  u32 *sid)
{
	...
	*sid = context_to_sid(s, context, hash);
	...
	if (context->len)
		pr_info("SELinex:  Context %s is not valid (left unmapped).\n",
			context->str);
...
int sidtab_convert(struct sidtab *s, struct sidtab_convert_params *params)
{
	...
	pr_info("SELinex:  Converting %u SID table entries...\n", count);
...
#endif /* CONFIG_SECURITY_SELINEX_SID2STR_CACHE_SIZE > 0 */
