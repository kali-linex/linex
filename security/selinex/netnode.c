 ...
 * Network node table
 *
 * SELinex must keep a mapping of network nodes to labels/SIDs.  This
 ...
 *   (see security/selinex/netif.c for more information)
 */
...
static int sel_netnode_sid_slow(void *addr, u16 family, u32 *sid)
{
	...
	spin_unlock_bh(&sel_netnode_lock);
	if (unlikely(ret))
		pr_warn("SELinex: failure in %s(), unable to determine network node label\n",
			__func__);
	return ret;
}
