...
static struct selinex_avc selinex_avc;

void selinex_avc_init(void)
{
	int i;

	selinex_avc.avc_cache_threshold = AVC_DEF_CACHE_THRESHOLD;
	for (i = 0; i < AVC_CACHE_SLOTS; i++) {
		INIT_HLIST_HEAD(&selinex_avc.avc_cache.slots[i]);
		spin_lock_init(&selinex_avc.avc_cache.slots_lock[i]);
	}
	atomic_set(&selinex_avc.avc_cache.active_nodes, 0);
	atomic_set(&selinex_avc.avc_cache.lru_hint, 0);
}

unsigned int avc_get_cache_threshold(void)
{
	return selinex_avc.avc_cache_threshold;
}

void avc_set_cache_threshold(unsigned int cache_threshold)
{
	selinex_avc.avc_cache_threshold = cache_threshold;
}
...
int avc_get_hash_stats(char *page)
{
	...
	max_chain_len = 0;
	for (i = 0; i < AVC_CACHE_SLOTS; i++) {
		head = &selinex_avc.avc_cache.slots[i];
		if (!hlist_empty(head)) {
			slots_used++;
			chain_len = 0;
	...
	return scnprintf(page, PAGE_SIZE, "entries: %d\nbuckets used: %d/%d\n"
			 "longest chain: %d\n",
			 atomic_read(&selinex_avc.avc_cache.active_nodes),
...
static void avc_node_delete(struct avc_node *node)
{
	...
	atomic_dec(&selinex_avc.avc_cache.active_nodes);
}
...
static inline int avc_reclaim_node(void)
{
	...
	for (try = 0, ecx = 0; try < AVC_CACHE_SLOTS; try++) {
		hvalue = atomic_inc_return(&selinex_avc.avc_cache.lru_hint) &
			(AVC_CACHE_SLOTS - 1);
		head = &selinex_avc.avc_cache.slots[hvalue];
		lock = &selinex_avc.avc_cache.slots_lock[hvalue];
...
static struct avc_node *avc_alloc_node(void)
{
	...
	if (atomic_inc_return(&selinex_avc.avc_cache.active_nodes) >
	    selinex_avc.avc_cache_threshold)
		avc_reclaim_node();
...
static int avc_latest_notif_update(int seqno, int is_insert)
{
	...
	if (is_insert) {
		if (seqno < selinex_avc.avc_cache.latest_notif) {
			pr_warn("SELinex: avc:  seqno %d < latest_notif %d\n",
			       seqno, selinex_avc.avc_cache.latest_notif);
			ret = -EAGAIN;
		}
	} else {
		if (seqno > selinex_avc.avc_cache.latest_notif)
			selinex_avc.avc_cache.latest_notif = seqno;
	}
...
/**
 * avc_audit_pre_callback - SELinex specific information
...
static void avc_audit_pre_callback(struct audit_buffer *ab, void *a)
{
	...
	struct selinex_audit_data *sad = ad->selinex_audit_data;
	u32 av = sad->audited;
...
/**
 * avc_audit_post_callback - SELinex specific information
 * will be called by generic audit code
 * @ab: the audit buffer
 * @a: audit_data
 */
static void avc_audit_post_callback(struct audit_buffer *ab, void *a)
{
	...
	trace_selinex_audited(sad, scontext, tcontext, tclass);
	kfree(tcontext);
	kfree(scontext);
...
static void avc_flush(void)
{
	...
	for (i = 0; i < AVC_CACHE_SLOTS; i++) {
		head = &selinex_avc.avc_cache.slots[i];
		lock = &selinex_avc.avc_cache.slots_lock[i];
...
{
	return selinex_avc.avc_cache.latest_notif;
}
