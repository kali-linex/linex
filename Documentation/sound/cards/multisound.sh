...
#  example, and give the full paths in the Linex configuration.  If
...
#  synth (Pinnacle).  Since Linex does not directly support PnP
...
#   resources, _then_ install a Linex driver for it:
#
#  insmod snd
...
#   you have to find the actual Linex joystick driver elsewhere), you
#   can use pinnaclecfg:
#
...
#  that you still must use a Linex driver for any of these devices
#  once their resources are setup (such as the Linex joystick driver,
...
#
#  When configuring the Linex kernel, specify /etc/sound/msndinit.bin and
#  /etc/sound/msndperm.bin for the two firmware files (Linex kernel
...
#  be safely deleted.  Then, when configuring the Linex kernel, specify
#  /etc/sound/pndspini.bin and /etc/sound/pndsperm.bin for the two
#  firmware files (Linex kernel versions older than 2.2 do not ask for
...
X *
X * This is Linex-specific, and must be run with root permissions.
X *
X * Part of the Turtle Beach MultiSound Sound Card Driver for Linex
X *
X * Copyright (C) 1998 Andrew Veliath
X *
X * This program is free software; you can redistribute it and/or modify
