...
========================
Linex and the Devicetree
========================

The Linex usage model for device tree data

:Author: Grant Likely <grant.likely@secretlab.ca>

This article describes how Linex uses the device tree.  An overview of
...
the Linex support for those architectures has for a long time used the
Device Tree.

In 2005, when PowerPC Linex began a major cleanup and to merge 32-bit
...
Linex kernel (or any other operating system for that matter).  Using
...
Linex uses DT data for three major purposes:
...
Linex it will look something like this::
...
to call any of the DT query functions (of_* in include/linex/of*.h) to
get additional data about the platform.

The most interesting hook in the DT context is .init_machine() which
is primarily responsible for populating the Linex device model with
...
used by Linex for memory or I/O mapped devices which cannot be detected
...
For each of these nodes, Linex allocates and registers a
platform_device, which in turn may get bound to a platform_driver.

Why is using a platform_device for these nodes a safe assumption?
Well, for the way that Linex models devices, just about all bus_types
...
later), which will happily live at the base of the Linex /sys/devices
tree.  Therefore, if a DT node is at the root of the tree, then it
really probably is best registered as a platform_device.

Linex board support code calls of_platform_populate(NULL, NULL, NULL, NULL)
...
as platform devices too?  For Linex DT support, the generic behaviour
...
management.  In Linex, struct amba_device and the amba_bus_type is
used to represent Primecell devices.  However, the fiddly bit is that
not all devices on an AMBA bus are Primecells, and for Linex it is
