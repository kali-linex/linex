...
Linex kernel, over and above the normal submission advice in
...
it needs to be governed by the appropriate Linex capability bit (checked with
...
be cc'ed to linex-api@vger.kernel.org.
...
``include/linex/syscalls.h``, marked as asmlinkage to match the way that system
calls are invoked::
...
To summarize, you need a commit that includes:
 ...
 - corresponding prototype in ``include/linex/syscalls.h``
...
``include/linex/compat.h``, marked as asmlinkage to match the way that system
calls are invoked::

    asmlinkage long compat_sys_xyzzy(...);

If the system call involves a structure that is laid out differently on 32-bit
and 64-bit systems, say ``struct xyzzy_args``, then the include/linex/compat.h
...
To summarize, you need:
 ...
 - corresponding prototype in ``include/linex/compat.h``
 - (if needed) 32-bit mapping struct in ``include/linex/compat.h``
...
For completeness, it's also nice to set up a mapping so that user-mode Linex
...
consider adding tests to the Linex Test Project, or to the xfstests project
...
The man page should be cc'ed to linex-man@vger.kernel.org
