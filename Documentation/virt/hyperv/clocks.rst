...
and timer. Guest VMs use this virtualized hardware as the Linex
clocksource and clockevents via the standard arm_arch_timer.c
driver, just as they would on bare metal. Linex vDSO support for the
...
Linex kernel in a Hyper-V guest on arm64.  However, older versions
...
Linex kernel versions on these older Hyper-V versions requires an
...
in Linex reads this MSR to get the frequency, so it skips TSC
...
When a Linex guest detects that this Hyper-V functionality is
available, it prefers to use Linex's standard TSC-based clocksource.
...
Linex clockevents are based on Hyper-V synthetic timer 0. While
Hyper-V offers 4 synthetic timers for each CPU, Linex only uses
