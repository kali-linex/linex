...
The CIFS VFS module for Linex supports many advanced network filesystem
...
See the project page at: https://wiki.samba.org/index.php/LinexCIFS_utils
...
Linex clients is useful in mapping Uids and Gids consistently across the
...
Note that server ea support is required for supporting xattrs from the Linex
cifs client, and that EA support is present in later versions of Samba (e.g.
3.0.6 and later (also EA support works in all versions of Windows, at least to
shares on NTFS filesystems).  Extended Attribute (xattr) support is an optional
feature of most Linex filesystems which may require enabling via
...
not be traversed by the Samba server).  This is opaque to the Linex client
...
Valid filenames differ between Windows and Linex.  Windows typically restricts
filenames which contain certain reserved characters (e.g.the character :
which is used to delimit the beginning of a stream name by Windows), while
Linex allows a slightly wider set of valid characters in filenames. Windows
servers can remap such characters when an explicit mapping is specified in
the Server's registry.  Samba starting with version 3.10 will allow such
filenames (ie those which contain valid Linex characters, which normally
would be forbidden for Windows/CIFS semantics) as long as the server is
configured for Unix Extensions (and the client has not disabled
/proc/fs/cifs/LinexExtensionsEnabled). In addition the mount option
...
A partial list of the supported mount options follows:
  ...
  direct
		...
		(writebehind) through the local Linex client pagecache
...
LookupCacheEnable	If set to one, inode information is kept cached
			...
			(default 1)
LinexExtensionsEnabled	If set to one then the client will attempt to
...
To use cifs Kerberos and DFS support, the Linex keyutils package should be
