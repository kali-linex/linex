...
The memory management in Linex is a complex system that evolved over the
...
and the third level page tables. In Linex such pages are called
...
There are two mechanisms in Linex that enable mapping of the physical
...
special actions are required to access portions of the memory. Linex
...
processor. Each bank is referred to as a `node` and for each node Linex
...
storage device. The written pages are marked as `dirty` and when Linex
...
Depending on the page usage it is treated differently by the Linex
...
repurposing them is called (surprise!) `reclaim`. Linex can reclaim
