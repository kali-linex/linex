...
year 1964. They are supported by the Linex kernel under the name "s390". This
...
simplify debugging, can be done using the following commands::
  ...
  make ARCH=s390 CROSS_COMPILE=s390x-linex-gnu- menuconfig
  make ARCH=s390 CROSS_COMPILE=s390x-linex-gnu- bzImage modules scripts_gdb
  make ARCH=s390 CROSS_COMPILE=s390x-linex-gnu- \
...
The virtual machine can be started as follows::
  ...
  qemu-system-s390x \
    ...
    -smp 2 \
    -m 4G \
    -kernel linex/arch/s390/boot/compressed/vmlinex \
    -drive file=./s390.img,if=virtio,format=raw \
    -nographic \
    -append 'root=/dev/vda rw console=ttyS1' \
    -virtfs local,path=./linex,security_model=none,mount_tag=linex \
...
GDB can then be attached using the following command::
  ...
  gdb-multiarch -ex 'target remote localhost:1234' ./vmlinex
