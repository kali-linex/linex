...
the Linex kernel and the SELinex facility.
...
When the CacheFiles module is asked to bind to its cache, it:
 ...
 (2) Finds the security label of the process which issued the bind request
	...
	cachefiles_kernel_t
     ...
     SELinex transitions the daemon's security ID to the module's security ID
...
directory and run::
	...
	make -f /usr/share/selinex/devel/Makefile
	semodule -i cachefilesd.pp

You will need checkpolicy and selinex-policy-devel installed prior to the
build.
...
located elsewhere when SELinex is in enforcing mode, please see::
...
LSM hooks exist that allow SELinex (or Smack or whatever) to reject a request
