...
provided by site administrators, or tools provided by Linex
distributors.
...
By default, the Linex NFSv4 client implementation constructs its
"co_ownerid" string starting with the words "Linex NFS" followed by
...
unexpectedly. Problematic situations include:
  ...
  - "Containers" within a single Linex host.  If each container has
...
Linex provides two mechanisms to add uniqueness to its "co_ownerid"
string:
    ...
    /sys/fs/nfs/net/nfs_client/identifier
      This virtual file, available since Linex 5.3, is local to the
...
If NFS with Kerberos is not configured, a Linex NFSv4 client uses
...
When a Kerberos keytab is present on a Linex NFS client, the client
...
Additionally, the Linex NFS client uses the RPCSEC_GSS security
flavor with Kerberos and the integrity QOS to prevent in-transit
modification of lease modification requests.

Additional notes
----------------
The Linex NFSv4 client establishes a single lease on each NFSv4
server it accesses. NFSv4 mounts from a Linex NFSv4 client of a
...
running applications. The Linex NFSv4 client facilitates state
