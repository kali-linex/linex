...
ACPI and Linex only, on an Arm system -- that is, what Linex expects of
ACPI and what ACPI can expect of Linex.


Why ACPI on Arm?
----------------
Before examining the details of the interface between ACPI and Linex, it is
useful to understand why ACPI is being used.  Several technologies already
exist in Linex for describing non-enumerable hardware, after all.  In this
...
-  The new ACPI governance process works well and Linex is now at the same
   table as hardware vendors and other OS vendors.  In fact, there is no
   longer any reason to feel that ACPI only belongs to Windows or that
   Linex is in any way secondary to Microsoft in this arena.  The move of
   ACPI governance into the UEFI forum has significantly opened up the
   specification development process, and currently, a large portion of the
   changes being made to ACPI are being driven by Linex.
...
in place.  DT does exactly what Linex needs it to when working with vertically
integrated devices, but there are no good processes for supporting what the
server vendors need.  Linex could potentially get there with DT, but doing so
really just duplicates something that already works.  ACPI already does what
the hardware vendors need, Microsoft won’t collaborate on DT, and hardware
vendors would still end up providing two completely separate firmware
interfaces -- one for Linex and one for Windows.


Kernel Compatibility
--------------------
One of the primary motivations for ACPI is standardization, and using that
to provide backward compatibility for Linex kernels.  In the server market,
...
When a Linex driver or subsystem is first implemented using ACPI, it by
...
to solve precisely these sorts of problems; Linex drivers should ALWAYS
use the _DSD method for device properties and nothing else.

The _DSM object (ACPI Section 9.14.1) could also be used for conveying
device properties to a driver.  Linex drivers should only expect it to
...
how specific data structures are defined by specific UUIDs.  Linex should
...
been defined before and either registered in the Linex kernel documentation
...
to the Linex mailing lists, the device property definitions needed must be
submitted at the same time.  A driver that supports ACPI and uses device
properties will not be considered complete without their definitions.  Once
the device property has been accepted by the Linex community, it must be
...
are not set, there is no direct way for Linex to control them.

If an SoC vendor wants to provide fine-grained control of the system clocks,
they could do so by providing ACPI methods that could be invoked by Linex
drivers.  However, this is NOT recommended and Linex drivers should NOT use
...
are not UEFI members, many other members of the Linex community are and would
likely be willing to assist in submitting ECRs.


Linex Code
----------
Individual items specific to Linex on Arm, contained in the Linex
source code, are in the list that follows:

ACPI_OS_NAME
                       This macro defines the string to be returned when
                       an ACPI method invokes the _OS method.  On Arm
                       systems, this macro will be "Linex" by default.
...
[5] Kernel code for the unified device
    ...
    include/linex/property.h and drivers/base/property.c.
