...
Linex kernel for ARC processors
*******************************
...
are supported in Linex. Some options are transparent to software
...
is to run Linex. SoC design teams should refer to "Appendix E:
Configuration for ARC Linex" in the ARC HS Databook for configurability
guidelines.
...
Building the Linex kernel for ARC processors
############################################
...
- Linex kernel compilers collection:
...
``CROSS_COMPILE=arc-linex`` (or whatever matches installed ARC toolchain prefix)
and then as usual ``make defconfig && make``.

This will produce "vmlinex" file in the root of the kernel source tree
