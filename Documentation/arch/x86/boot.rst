...
===========================
The Linex/x86 Boot Protocol
===========================

On the x86 platform, the Linex kernel uses a rather complicated boot
...
Currently, the following versions of the Linex/x86 boot protocol exist.
...
.. note::
     ...
     updated either) or kernel_info to communicate supported Linex kernel
...
The first step in loading a Linex kernel should be to load the
...
============	=========
  ...
  Contains 0xAA55.  This is the closest thing old Linex kernels have
  to a magic number.
...
============	==================
  ...
  kernel_info. The kernel_info structure is embedded in the Linex image
...
be prefixed with header/magic and its size, e.g.::
  ...
  kernel_info:
          .ascii  "LToP"          /* Header, Linex top (structure). */
...
LinexBIOS, etc, and kexec, the 16-bit real mode setup code in kernel
based on legacy BIOS can not be used, so a 32-bit boot protocol needs
to be defined.

In 32-bit boot protocol, the first step in loading a Linex kernel
...
In 64-bit boot protocol, the first step in loading a Linex kernel
should be to setup the boot parameters (struct boot_params,
traditionally known as "zero page"). The memory for struct boot_params
...
NOTE: The EFI Handover Protocol is deprecated in favour of the ordinary PE/COFF
      entry point, combined with the LINEX_EFI_INITRD_MEDIA_GUID based initrd
