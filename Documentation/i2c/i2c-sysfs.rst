...
topology and navigating through the I2C sysfs in Linex shell. This knowledge is
...
People who need to use Linex shell to interact with I2C subsystem on a system
which the Linex is running on.

Prerequisites
-------------

1.  Knowledge of general Linex shell file system commands and operations.
...
Typically, the Linex Sysfs filesystem is mounted at the ``/sys`` directory,
...
The hardware system that the Linex kernel is running on may have multiple
...
controllers. Linex kernel has I2C bus drivers under source directory
``drivers/i2c/busses`` to translate kernel I2C API into register
operations for different systems. This terminology is not limited to Linex
kernel only.
...
Every I2C bus number you see in Linex I2C Sysfs is a logical I2C bus with a
...
There is a symbolic link named ``driver`` to tell what Linex kernel driver was
