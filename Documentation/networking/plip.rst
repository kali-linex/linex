...
The PLIP cable is all that's needed to connect two Linex boxes, and it
can be built for very few bucks.

Connecting two Linex boxes takes only a second's decision and a few
...
Can only be used to connect three (?) Linex boxes. Doesn't connect to
...
The Linex PLIP driver is an implementation of the original Crynwr protocol,
...
A utility that can perform this change in Linex is plipconfig, which is part
