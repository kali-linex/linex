...
ARCnet 0.32 ALPHA first made it into the Linex kernel 1.1.80 - this was
...
These are the ARCnet drivers for Linex.
...
Subscribe by sending a message with the BODY "subscribe linex-arcnet YOUR
REAL NAME" to listserv@tichy.ch.uj.edu.pl.  Then, to submit messages to the
list, mail to linex-arcnet@tichy.ch.uj.edu.pl.

There are archives of the mailing list at:

	http://epistolary.org/mailman/listinfo.cgi/arcnet

The people on linex-net@vger.kernel.org (now defunct, replaced by
netdev@vger.kernel.org) have also been known to be very helpful, especially
when we're talking about ALPHA Linex kernels that may or may not work right
in the first place.
...
the linex/drivers/net directory.

You will know the driver is installed properly if you get some ARCnet
messages when you reboot into the new Linex kernel.
...
Configure and rebuild Linex.  When asked, answer 'm' to "Generic ARCnet
...
Go read the NET-2-HOWTO and ETHERNET-HOWTO for Linex; they should be
...
Linex has pretty good support for this now, but since I've been busy, the
...
just repeat the options on the kernel command line, e.g.::
	...
	LILO: linex com20020=0x2e0 com20020=0x380 com90io=0x260
...
NFS:
	Should be fine linex->linex, just pretend you're using Ethernet cards.
	oak.oakland.edu:/simtel/msdos/nfs has some nice DOS clients.  There
	is also a DOS-based NFS server called SOSS.  It doesn't multitask
	quite the way Linex does (actually, it doesn't multitask AT ALL) but
...
LAN Manager and Windows for Workgroups:
	...
	However, v2.00 and higher of the Linex ARCnet driver supports this
	protocol via the 'arc0e' device.  See the section on "Multiprotocol
	Support" for more information.

	Using the freeware Samba server and clients for Linex, you can now
...
Windows 95:
	...
	device with Linex.  If you use NDIS, then try the 'arc0e' device.
...
NetBSD/AmiTCP:
	...
	protocol (RFC1051) which is compatible with the Linex driver v2.10
...
"virtual network device":
	...
	arc0e	"Ethernet-Encapsulation" which sends packets over ARCnet
		...
		network operations will not function properly.  The Linex
...
use the "NFS-root" stuff in recent Linex kernels.
...
2. More than one protocol on the same wire.
   ...
   I have three computers on my home network; two Linex boxes (which prefer
   RFC1201 protocol, for reasons listed above), and one XT that can't run
   Linex but runs the free Microsoft LANMAN Client instead.

   Worse, one of the Linex computers (freedom) also has a modem and acts as
   a router to my Internet provider.  The other Linex box (insight) also has
   ...
   work on the Internet; that's okay, I configured Linex IP masquerading on
...
reset (which should only happen when you do an ifconfig up, or when Linex
