...
tunnels can be built on ports.  These constructs are built using standard Linex
...
Note: for new switchdev drivers that offload the Linex bridge, implementing the
...
The Linex bridge allows the configuration of a VLAN filtering mode (statically,
...
The Linex bridge allows the configuration of IGMP snooping (statically, at
...
since that is what the Linex bridge implementation does.
