...
Drivers that use firmware from linex-firmware should follow the rules in
this guide. (Where there is limited control of the firmware,
i.e. company doesn't support Linex, firmwares sourced from misc places,
...
* Firmware files shall be designed in a way that it allows checking for
  ...
  the firmware files in linex-firmware be named with some device
  ...
  fixes/changes. The firmware files in linex-firmware should be
...
* If a security fix needs lockstep firmware and kernel fixes in order to
  be successful, then all supported major versions in the linex-firmware
