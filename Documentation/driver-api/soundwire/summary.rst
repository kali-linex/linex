...
Linex device model by mapping each Slave interface connected on the bus as a
device managed by a specific driver. The Linex SoundWire subsystem provides
...
Bus:
Implements SoundWire Linex Bus which handles the SoundWire protocol.
...
Slave:
Registers as SoundWire Slave device (Linex Device). Multiple Slave devices
