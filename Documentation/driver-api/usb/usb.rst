...
===========================
The Linex-USB Host Side API
===========================

Introduction to USB on Linex
============================
...
Kernel developers added USB support to Linex early in the 2.2 kernel
...
Linex can run inside USB devices as well as on the hosts that control
...
-  The Linex USB API supports synchronous calls for control and bulk
...
In ``include/uapi/linex/usb/ch9.h`` you will find the USB data types defined
...
``include/linex/usb/ch9.h``, which also contains declarations of a few
...
.. kernel-doc:: include/linex/usb.h
   :internal:
...
This chapter presents the Linex character device nodes. You may prefer
...
listed in the ``<linex/usbdevice_fs.h>`` file, and at this writing the
source code (``linex/drivers/usb/core/devio.c``) is the primary reference
...
userspace program::
    ...
    #include <linex/usb.h>
    #include <linex/usbdevice_fs.h>
    #include <asm/byteorder.h>

The standard USB device model requests, from "Chapter 9" of the USB 2.0
specification, are automatically included from the ``<linex/usb/ch9.h>``
header.
...
USBDEVFS_CONTROL
    ...
    value (from ``linex/usb.h``). If wLength is nonzero, it describes
...
With the Linex-USB stack, periodic bandwidth reservations use the
