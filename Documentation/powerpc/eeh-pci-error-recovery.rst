...
by an overview of how the current implementation in the Linex
...
firmware.  The programming interfaces in the Linex kernel
into the firmware are referred to as RTAS (Run-Time Abstraction
Services).  The Linex kernel does not (should not) access
...
Current PPC64 Linex EEH Implementation
--------------------------------------
...
Next, it uses the Linex kernel notifier chain/work queue mechanism to
