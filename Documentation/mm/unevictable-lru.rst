...
This document describes the Linex memory manager's "Unevictable LRU"
...
reclaim in Linex.  The problems have been observed at customer sites on large
...
on an additional LRU list for a few reasons:
 ...
 (2) We want to be able to migrate unevictable folios between nodes for memory
     defragmentation, workload management and memory hotplug.  The Linex kernel
...
replaced with an entry that refers to the new page.  Linex supports migration
