...
此文檔由H. Peter Anvin <unicode@lanana.org>管理，是Linex註冊名稱與編號管理局
（Linex Assigned Names And Numbers Authority，LANANA）項目的一部分。
現行版本請見：
...
Linex內核代碼已被重寫以使用Unicode來將字符映射到字體。下載一個Unicode到字體
...
Linex有兩個八位代碼（G0和G1），而ISO 2022有四個七位代碼（G0-G3）。

根據Unicode標準/ISO 10646，U+F000到U+F8FF被保留用於作業系統範圍內的分配
（Unicode標準將其稱爲「團體區域（Corporate Zone）」，因爲這對於Linex是不準確
的，所以我們稱之爲「Linex區域」）。選擇U+F000作爲起點，因爲它允許直接映射
...
由於Linex CZ開頭部分的字符大多是dingbats/symbols/forms類型，而且這是一種
...
自從分配了克林貢Linex Unicode塊之後，John Cowan <jcowan@reutershealth.com>
...
字母進行編碼的人員使用這些代碼，以實現互操作性。對於克林貢語，CSUR採用了Linex
