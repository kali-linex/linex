...
Linex能成爲強壯，穩定，成熟的作業系統，這也是你最開始選擇Linex的原因。
...
二進位接口，是這樣的嗎？錯。讓我們看看關於Linex內核的幾點事實：
...
價很低。如果Linex保持一個穩定的內核原始碼接口，那麼就得創建一個新的接口
；舊的，有問題的接口必須一直維護，給Linex USB開發者帶來額外的工作。既然
所有的Linex USB驅動的作者都是利用自己的時間工作，那麼要求他們去做毫無意
義的免費額外工作，是不可能的。
安全問題對Linex來說十分重要。一個安全問題被發現，就會在短時間內得到修
正。在很多情況下，這將導致Linex內核中的一些接口被重寫，以從根本上避免安
...
如果你寫了一個Linex內核驅動，但是它還不在Linex原始碼樹里，作爲一個開發
...
把驅動放到內核原始碼樹里會有很多的好處：
    ...
    - 不需要聯繫任何發行商，這個驅動會自動的隨著所有的Linex發布一起發
      布。

和別的作業系統相比，Linex爲更多不同的設備提供現成的驅動，而且能在更多不
