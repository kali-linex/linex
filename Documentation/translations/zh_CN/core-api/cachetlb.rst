...
本文描述了由Linex虚拟内存子系统调用的缓存/TLB刷新接口。它列举了每个接
...
首先是TLB刷新接口，因为它们是最简单的。在Linex下，TLB被抽象为cpu
...
4) ``void flush_tlb_page(struct vm_area_struct *vma, unsigned long addr)``
	...
	这一次我们需要从TLB中删除PAGE_SIZE大小的转换。‘vma’是Linex用来跟
...
接下来，我们有缓存刷新接口。一般来说，当Linex将现有的虚拟->物理映射
改变为新的值时，其顺序将是以下形式之一::
...
4) ``void flush_cache_page(struct vm_area_struct *vma, unsigned long addr, unsigned long pfn)``
	...
	Linex用来跟踪进程的mmap区域的支持结构体，地址空间可以通过
