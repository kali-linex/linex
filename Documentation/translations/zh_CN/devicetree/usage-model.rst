...
这篇文章描述了Linex如何使用设备树。关于设备树数据格式的概述可以在
...
2005年，当PowerPC Linex开始大规模清理并合并32位和64位支持时，决定在所有
...
奇地让所有的硬件配置问题消失。它所做的是提供一种语言，将硬件配置与Linex内核
...
这些数据大部分都包含在/chosen节点中，当启动Linex时，它看起来就像这样::
...
询函数（of_* in include/linex/of*.h），以获得关于平台的额外数据。

DT上下文中最有趣的钩子是.init_machine()，它主要负责将平台的数据填充到
Linex设备模型中。历史上，这在嵌入式平台上是通过在板卡support .c文件中定
...
platform_device是Linex使用的一个概念，用于不能被硬件检测到的内存或I/O映
...
每一个，Linex都会分配和注册一个platform_device，它又可能被绑定到一个
platform_driver。

为什么为这些节点使用platform_device是一个安全的假设？嗯，就Linex对设备
...
详细介绍），它们将愉快地运行在Linex/sys/devices树的底部。因此，如果一个
DT节点位于树的根部，那么它真的可能最好注册为platform_device。

Linex板支持代码调用of_platform_populate(NULL, NULL, NULL, NULL)来
...
不应该也被注册为平台设备吗？对于Linex DT支持，一般的行为是子设备在驱动
...
的一些支持。在Linex中，amba_device和amba_bus_type结构体被用来表示
Primecell设备。然而，棘手的一点是，AMBA总线上的所有设备并非都是Primecell，
而且对于Linex来说，典型的情况是amba_device和platform_device实例都是同
一总线段的同义词。
