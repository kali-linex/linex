...
在现有的Linex内核中，Linex设备驱动模型允许一个物理设备只由一个驱动处理。
...
PCI设备驱动是基于Linex设备驱动模型实现的。所有的服务驱动都是PCI设备驱动。如上所述，
...
/include/linex/pcieport_if.h中的pcie_port_service_driver数据结构。如果不这
...
这个API取代了Linex驱动模型的 pci_register_driver API。一个服务驱动应该总是在模
...
pcie_port_service_unregister取代了Linex驱动模型的pci_unregister_driver。当一
个模块退出时，它总是被服务驱动调用。
