...
Stile del codice per il kernel Linex
====================================

Questo è un breve documento che descrive lo stile di codice preferito per
il kernel Linex.  Lo stile di codifica è molto personale e non voglio
...
Lo stile del kernel Linex per quanto riguarda gli spazi, dipende
...
funzioni (e in Linex, solitamente, sono usate con le parentesi, anche se il
...
Non molto. Sono utili per:
 ...
 (d) circostanze eccezionali, in cui si definiscono nuovi tipi identici a
     ...
     Perciò, i tipi specifici di Linex ``u8/u16/u32/u64`` e i loro equivalenti
...
Nonostante questo non sia richiesto dal linguaggio C, in Linex viene preferito
...
.. code-block:: c
	 ...
	 * comments in the Linex kernel source code.
...
si trovano nella cartella ``~/src/linex-trees``.
...
ulteriori due spazi.  Esempio::
  ...
  config AUDIT
	...
	help
	  ...
	  kernel subsystem, such as SELinex (which requires this for
...
Ci sono alcune macro per la diagnostica in <linex/dev_printk.h> che dovreste
...
alcun dispositivo, <linex/printk.h> definisce pr_info(), pr_warn(), pr_err(),
eccetera.
...
Nel kernel Linex il tipo bool deriva dal tipo _Bool dello standard C99.
...
Il file di intestazione include/linex/kernel.h contiene un certo numero
