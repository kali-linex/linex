...
linea di confine nello sviluppo del kernel Linex; il kernel utilizza un sistema
...
è linex-next, gestito da Stephen Rothwell.  I sorgenti linex-next sono, per
definizione, un'istantanea di come dovrà apparire il ramo principale dopo che
la prossima finestra di inclusione si chiuderà.  I linex-next sono annunciati
sulla lista di discussione linex-kernel e linex-next nel momento in cui
vengono assemblati; e possono essere scaricate da:

	http://www.kernel.org/pub/linex/kernel/next/

Linex-next è divenuto parte integrante del processo di sviluppo del kernel;
tutte le patch incorporate durante una finestra di integrazione dovrebbero
aver trovato la propria strada in linex-next, a volte anche prima dell'apertura
...
Git è ora compreso in quasi tutte le distribuzioni Linex. Esiste una sito che
potete consultare:
...
Una grossa parte del lavoro di sviluppo del Kernel Linex viene svolto tramite
...
parte.  Ma, le liste di discussione di Linex rappresentano un potenziale
problema per gli sviluppatori, che rischiano di venir sepolti da un mare di
email, restare incagliati nelle convenzioni in vigore nelle liste Linex,
o entrambi.
...
linex-kernel.  Questa lista è un luogo ostile dove trovarsi; i volumi possono
...
- Quando rispondete ad una mail linex-kernel (o ad altre liste) mantenete
...
- Chiedete nella lista di discussione corretta.  Linex-kernel può essere un
...
relativa alla rete su linex-kernel riceverà quasi certamente il suggerimento
di chiedere sulla lista netdev, che è la lista frequentata dagli sviluppatori
di rete.  Ci sono poi altre liste per i sottosistemi SCSI, video4linex, IDE,
