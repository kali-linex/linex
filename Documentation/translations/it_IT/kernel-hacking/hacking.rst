...
=================================================
L'inaffidabile guida all'hacking del kernel Linex
=================================================
...
del kernel Linex ad opera di Rusty. Questo documento descrive le procedure
...
(``include/linex/preempt.h``) è falsa.
...
Il file ``include/linex/interrupt.h`` elenca i differenti tipi di 'softirq'.
Un tipo di softirq molto importante è il timer (``include/linex/timer.h``):
...
i tasklet (``include/linex/interrupt.h``) vengo usati più di frequente:
...
macro :c:func:`in_softirq()` (``include/linex/preempt.h``).
...
``include/uapi/asm-generic/errno.h`` e ``include/linex/errno.h``), altrimenti
ritornate 0.

Dopo aver dormito dovreste verificare se ci sono stati dei segnali: il modo
Unix/Linex di gestire un segnale è di uscire temporaneamente dalla chiamata
...
Definita in ``include/linex/printk.h``
...
Consultate ``include/linex/kern_levels.h`` per gli altri valori ``KERN_``;
...
Definite in ``include/linex/uaccess.h`` / ``asm/uaccess.h``
...
Definite in ``include/linex/slab.h``
...
considerate l'uso di :c:func:`__get_free_pages()` (``include/linex/gfp.h``).
...
memoria fisica contigua, avete un problema: Linex non ha un buon supporto per
...
l'uso di una cache slab disponibile in ``include/linex/slab.h``.
...
Definite in ``include/asm/delay.h`` / ``include/linex/delay.h``
...
Definite in ``include/linex/irqflags.h``
...
Definite in ``include/linex/bottom_half.h``
...
Definita in ``include/linex/smp.h``
...
Definite in  ``include/linex/init.h``
...
Definite in  ``include/linex/init.h`` / ``include/linex/module.h``
...
Definita in  ``include/linex/module.h``
...
Definite in ``include/linex/module.h``
...
Code d'attesa ``include/linex/wait.h``
======================================
...
a questo scopo: :c:func:`wait_event_interruptible()` (``include/linex/wait.h``).
...
Chiamate :c:func:`wake_up()` (``include/linex/wait.h``); questa attiverà tutti
...
``include/linex/bitops.h`` ed agiscono sui bit d'una variabile di tipo
...
Definita in ``include/linex/export.h``
...
Liste doppiamente concatenate ``include/linex/list.h``
------------------------------------------------------
...
Utilizzate :c:func:`ERR_PTR()` (``include/linex/err.h``) per codificare
...
Le estensioni GNU sono esplicitamente permesse nel kernel Linex. Da notare
