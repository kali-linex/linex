...
struct sbmac_softc {
	...
	/*
	 * Linex-specific things
	 */
	struct net_device	*sbm_dev;	/* pointer to linex device */
...
static int sbdma_add_txbuffer(struct sbmacdma *d, struct sk_buff *sb)
{
	...
	/*
	 * Under Linex, it's not necessary to copy/coalesce buffers
 ...
 *  SBMAC_INIT(dev)
 *
 *  Attach routine - init hardware and hook ourselves into linex
 *
 *  Input parameters:
