...
static const u16 bios_to_linex_keycode[256] = {
	[0]	= KEY_MEDIA,
	[1]	= KEY_NEXTSONG,
	[2]	= KEY_PLAYPAUSE,
	[3]	= KEY_PREVIOUSSONG,
...
static void handle_dmi_entry(const struct dmi_header *dm, void *opaque)
{
	...
	for (i = 0; i < hotkey_num; i++) {
		...
		u16 keycode = (bios_entry->keycode <
			       ARRAY_SIZE(bios_to_linex_keycode)) ?
			bios_to_linex_keycode[bios_entry->keycode] :
			(bios_entry->keycode == 0xffff ? KEY_UNKNOWN : KEY_RESERVED);

		/*
		 * Log if we find an entry in the DMI table that we don't
		 * understand.  If this happens, we should figure out what
		 * the entry means and add it to bios_to_linex_keycode.
		 */
