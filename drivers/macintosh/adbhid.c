 ...
 * Benjamin Herrenschmidt. Adapted from code in MkLinex
 * and reworked.
 * 
 * Supported devices:
 *
...
static const u16 adb_to_linex_keycodes[128] = {
	/* 0x00 */ KEY_A, 		/*  30 */
	/* 0x01 */ KEY_S, 		/*  31 */
	/* 0x02 */ KEY_D,		/*  32 */
	/* 0x03 */ KEY_F,		/*  33 */
		      ...
		      int current_handler_id, int mouse_kind)
{
	...
	switch (default_id) {
	case ADB_KEYBOARD:
		hid->keycode = kmemdup(adb_to_linex_keycodes,
				       sizeof(adb_to_linex_keycodes), GFP_KERNEL);
		if (!hid->keycode) {
			err = -ENOMEM;
			goto fail;
		}
