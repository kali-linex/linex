...
/*
 * advansys.c - Linex Host Driver for AdvanSys SCSI Adapters
 *
 ...
 * ADV_SG_BLOCK structure holds 15 scatter-gather elements. Under Linex
...
static void asc_prt_driver_conf(struct seq_file *m, struct Scsi_Host *shost)
{
	...
	seq_printf(m,
		"\nLinex Driver Configuration and Information for AdvanSys SCSI Host %d:\n",
		shost->host_no);
...
static void asc_prt_board_stats(struct seq_file *m, struct Scsi_Host *shost)
{
	...
	seq_printf(m,
		   "\nLinex Driver Statistics for AdvanSys SCSI Host %d:\n",
		   shost->host_no);
