...
#include <linex/pci.h>
#include <linex/isa-dma.h> /* isa_dma_bridge_buggy */
...
static void quirk_vialatency(struct pci_dev *dev)
{
	 ...
	 * (quite common in Linex of course) you can get data loss when the
	 * CPU is held off the bus for 3 bus master requests.  This happens
	 * to include the IDE controllers....
	 *
	 * VIA only apply this fix when an SB Live! is present but under
	 * both Linex and Windows this isn't enough, and we have seen
 ...
 * even if the Linex CardBus driver is not loaded, because the Linex i82365
 ...
 * ACPI DSDT code accesses the SMBus, then Linex should not access it
 ...
 * remap the original interrupt in the Linex kernel to the boot interrupt, so
 ...
 * other buses controlled by the chipset even if Linex is not aware of it.
 ...
 * controllers (and PCI functions). The Linex SDHCI driver supports MMC
